...
- **Intelligent tutoring systems** — AI in the tutor role: next problem, evaluate, feedback. Domain model + student model, not a chatbot cadence. Practice after instruction lives elsewhere. PLATO/TUTOR is old; Cognitive Tutor / MATHia lineage.
- Also this module: collaborative tools (sociocultural), learner-centered management (e.g. spaced repetition), makerspaces/robotics (constructionism), emerging (genAI tutors, immersive).
- Cart/horse: tech is designed to travel across classrooms, so the *experience* around it is Module 3+.
...
**CBR vs analogy (lecture; do not collapse).** **CBR:** almost identical cases, small tweak. **Analogy:** domains can look unrelated; map **deep causal structure**. Duncker radiation + fortress. Bohr: atom :: solar system. Velcro / kingfisher-train. Failed cases still store (what *not* to do). LTM vs STM as in lecture / *Memento*.

**Images (Ch. 6).** Pictorial/spatial beats a long verbal description. Operations: inspect, find, **zoom, rotate, scan, transform**. Schema: images + those processes → behavior. **Kosslyn vs Pylyshyn:** is imagery real or only seeming-so? Thagard: rotation/scan experiments + neuro (imagery near perception) support real imagistic procedures. Lecture: rotation time scales with angle; childhood-house windows; method of loci.
